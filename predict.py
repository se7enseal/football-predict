...
def get_team_color(team_name, is_second=False):
    color = TEAM_COLORS.get(team_name, "#2E86C1") # 默认蓝色
    if is_second and color == "#DA291C": # 如果二队颜色和一队都是红色，强制切换为橙色
        return "#FF8C00"
    return color
...
def calculate_radar_metrics(df, team):
    ...
    state = (points / 15) * 10
    return {"进攻": round(attack,1), "防守": round(defence,1), "近期状态": round(state,1), "积分排名": 7.0}
...
if data is not None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.subheader(f"📊 {h_name} 表现")
        st.table(get_recent_results_df(data, h_name))
        h_injury = st.text_input(f"{h_name} 伤病:", "无")
    with c2:
        st.subheader(f"📊 {a_name} 表现")
        st.table(get_recent_results_df(data, a_name))
        a_injury = st.text_input(f"{a_name} 伤病:", "无")

    # 雷达图逻辑
    h_m, a_m = calculate_radar_metrics(data, h_name), calculate_radar_metrics(data, a_name)
    h_color, a_color = get_team_color(h_name), get_team_color(a_name, is_second=True)

    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=list(h_m.values()), theta=list(h_m.keys()), fill='toself', name=h_name, line_color=h_color))
    fig.add_trace(go.Scatterpolar(r=list(a_m.values()), theta=list(a_m.keys()), fill='toself', name=a_name, line_color=a_color))
    ...
    if st.sidebar.button("生成深度预测"):
        ...
        st.info(f"伤病警报: {h_name}({h_injury}) vs {a_name}({a_injury})")
else:
    st.error("数据加载失败。")
